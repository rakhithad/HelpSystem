...
const ManageUsers = () => {
    ...
    const [userRole, setUserRole] = useState(''); // To store current user's role
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch users
                const usersResponse = await axios.get(`${process.env.REACT_APP_BACKEND_BASEURL}/auth/users`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                setUsers(usersResponse.data);

                
                const userResponse = await axios.get(`${process.env.REACT_APP_BACKEND_BASEURL}/auth/account`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const handleSave = useCallback(async () => {
        try {
            ...
            setUsers((prevUsers) =>
                prevUsers.map((user) => (user._id === editingUserId ? { ...user, ...editedUser } : user))
            );
        } catch (error) {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await axios.delete(`${process.env.REACT_APP_BACKEND_BASEURL}/auth/users/${deleteUserId}`, {
            headers: { Authorization: `Bearer ${token}` },
        ...
        if (response.data.warning) {
            setError(response.data.warning); // Display warning about notification failure
        } else {
                ...
                <div className="p-4 lg:p-8 space-y-6">
                    ...
                    {isDeleteModalOpen && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                            <div className="bg-white bg-opacity-10 backdrop-blur-md p-6 rounded-2xl shadow-2xl max-w-md w-full">
                                <h2 className="text-xl font-bold text-purple-300 mb-4">Delete User</h2>
                                <p className="text-white text-opacity-80 mb-4">Please provide a reason for deleting this user:</p>
                                <textarea
                                    value={deleteReason}
                                    onChange={(e) => setDeleteReason(e.target.value)}
                                    maxLength={500}
                                    placeholder="Enter reason for deletion"
                                    className="w-full bg-white bg-opacity-20 text-white p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 mb-4"
                                />
                                <div className="flex justify-end gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 text-sm bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg hover:from-red-600 hover:to-pink-600 transition-all"
                                    >
                                        Confirm Delete
                                    </button>
                                </div>
                    ...
                    <div className="overflow-x-auto bg-white bg-opacity-10 backdrop-blur-md rounded-2xl shadow-2xl">
                        <table className="min-w-full table-auto text-sm text-left text-white text-opacity-90">
                            ...
                            <tbody>
                                {users.map((user) => (
                                    <tr key={user._id} className={`hover:bg-white hover:bg-opacity-10 transition-all duration-300 ${user.userStatus === 'inactive' ? 'opacity-50' : ''}`}>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {editingUserId === user._id ? (
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <path
                                                                d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828Etiotl8.38-8.379-2.83-2.828z"
                                                            />
                                                        </svg>
                                                        Edit
                                                    </button>
                                                )}
                                                {userRole === 'admin' && user.userStatus === 'active' && (
                                                    <button
